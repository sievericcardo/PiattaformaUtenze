$(document).ready(function () {
  $("form[name='db-cyb']").validate({
    ...
    submitHandler: function (form) {
      $.ajax({
        type: "POST",
        url: '<base_url>/piattaforma-utenze/resources/library/newCybDb.php',
        data: $('#db-cyb-form').serialize(),
        dataType: 'json',
        success: function (response) {
          ...
          if (obj.code == 200) {
            // window.location = '<base_url>/piattaforma-utenze/public/utenza/cyb/db';
          } else {
            toast (obj.message);
          }
        },
